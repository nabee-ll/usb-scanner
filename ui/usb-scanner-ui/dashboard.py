...
class DashboardPage(QWidget):
    ...
    def set_backend(self, backend):
        ...
        self.backend.device_detected.connect(self.on_device_detected)
        self.backend.prompt_received.connect(self.on_backend_prompt)
        self.backend.scan_complete.connect(self.on_scan_complete)

    def on_device_detected(self, device):
        # Update device info on dashboard
        self.lbl_status.setText(f"Secure Terminal: Detected {device.get('name', 'USB')}")
    ...
    def on_backend_prompt(self, prompt_id, payload):
        ...
        elif prompt_id in ["trust_hid", "trust_storage"]:
            pass # We'll need a popup for this later
    ...
    def handle_unknown_popup_action(self, action, selected_type, device):
        if action == "block":
            self.clear_popups()
            if self.backend:
                self.backend.send_response({"answer": "other"})
            self.device_blocked.emit(device)
        elif action == "scan":
            self.clear_popups()
            if self.backend:
                self.backend.send_response({"answer": selected_type})
            self.start_scanning_workflow(device)
